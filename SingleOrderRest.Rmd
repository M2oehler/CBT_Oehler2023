...
raw.data <- raw.file.paths %>%
  rowwise() %>%
  do(., read_table(file = .$filepath, skip = 12, col_names = c("reach", "STA", "day", "time", "wettedFPw")))


raw.files <- list.files("TextFiles/", pattern=".txt")
...
Q1_1st20p <- read_table("TextFiles/1-7_Q1_1st-20p.txt", skip = 12, col_names = c("reach", "STA", "day", "time", "wettedFPw")) %>% 
  drop_na(STA)%>%
  mutate(STA = 
           case_when(
...
#23353 one textfile no space 1st order river
#640181 for 1 plan all orders no spaces
