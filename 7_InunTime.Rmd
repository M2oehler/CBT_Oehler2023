...
#inun_ts_ALL has all scenarios for each river even when restoration is not within the specific river
# save(inun_ts_ALL, 
#     file = "C:/Users/morganao/Documents/CBT_Oehler2023/12_Saved_Dataframes/inun_ts_ALL.RData")
```

```{r}
#all scenarios for every river inundation time
inun_ts_ALL <- inun_ts%>%
  filter(ti_hr>0)
test <- inunVSrem %>%
  ungroup(storm, plan)%>%
  select(removal_scaled_kg,order)%>%
  bind_cols(inun_ts_ALL)%>%
  mutate(restLoc = 
           case_when(restLoc == "1st"~ "1-7",
                     restLoc == "2nd"~ "2-2",
                     restLoc == "3rd"~ "3-1",
                     restLoc == "4th"~ "4-1"))%>%
  filter(river == restLoc)
```


```{r plot-wider, fig.width=16, fig.height=4}
#inundation by time step method as opposed to summing inun time for each XS. Single order restoration scenarios.
filter1stO <-   RAS_1stO %>% #there is no inundation in the second day of the simulation.Filter that out and all rivers outside of the river where restoration occurs.
  filter(
    day == "01SEP2008" & river == "1-7")
filter2ndO <-   RAS_2ndO %>% 
  filter(
    day == "01SEP2008" & river == "2-2")
filter3rdO <-   RAS_3rdO %>% 
  filter(
    day == "01SEP2008" & river == "3-1")
filter4thO <-   RAS_4thO %>% 
  filter(
    day == "01SEP2008" & river == "4-1")

TEST_inun <- inun_ts%>%
  filter(plan == "4th-80p"|plan == "4th-100p")%>%
  filter(storm == "Q2"& river == "4-1")
...
inun_ts <- 
  bind_rows(RAS_1stO,
                     ...
                     RAS_3rdO,
                     RAS_4thO) %>%
  group_by(time, day, river, storm, plan)%>% 
  mutate(
    inun_ct = case_when(
      wettedFPw >10 ~ 1, #giving a unit value to every timestep & XS 
      TRUE~0))%>%
  summarise(
    inun_ct = sum(
      inun_ct, na.rm = TRUE))%>% #represents the number of XS that are inundated at that timestep. I am just using this to show what time steps have water in them within the reach.
  mutate(
    inun_int = case_when( #assigning a 15-min interval to each timestep with water in FP
      inun_ct >= 1~15, 
      TRUE~0))%>%
  group_by(storm, plan, river)%>%
  summarise(
    ti_hr = sum(inun_int)/60)%>% #value of inundation time in the reach for each plan and storm
  mutate(
    percentRest = case_when(
      endsWith(plan, "100p")~ 100, #for plotting purposes
...
inun_ts$storm <- factor(inun_ts$storm,                 # Re-level group factor
...
ts_inun_1st <- inun_ts%>%
  ...
  ggplot()+
  geom_line(aes(percentRest, ti_hr, color = storm, group = storm))+
  geom_point(aes(percentRest, ti_hr, color = storm))+
  scale_x_continuous(breaks=seq(0,100,by=20))+
  labs(x = expression(atop("Percent 1"^st*"-order streams restored")),
       y = expression(atop("Floodplain Inundation Time (hr)")),
       color = "Storm")+
...
ts_inun_2nd <- inun_ts%>%
  ...
  geom_point(aes(percentRest, ti_hr, color = storm))+
  scale_x_continuous(breaks=seq(0,100,by=20))+
  labs(x = expression(atop("Percent 2"^nd*"-order streams restored")),
       y = element_blank(),
       color = "Storm")+
...
ts_inun_3rd <- inun_ts%>%
  ...
  geom_point(aes(percentRest, ti_hr, color = storm))+
  scale_x_continuous(breaks=seq(0,100,by=20))+
  labs(x = expression(atop("Percent 3"^rd*"-order streams restored")),
       y = element_blank(),
       color = "Storm")+
...
ts_inun_4th <- inun_ts%>%
  ...
  geom_point(aes(percentRest, ti_hr, color = storm))+
  scale_x_continuous(breaks=seq(0,100,by=20))+
  labs(x = expression(atop("Percent 4"^th*"-order stream restored")),
       y = element_blank(),
       color = "Storm")+
  ggtitle(expression("4"^th*"-order River"))


ts_inun_1st+ts_inun_2nd+ts_inun_3rd+ts_inun_4th +
   plot_layout(ncol = 4, guides = "collect")+
             plot_annotation(title = 
                               expression(atop('Floodplain Inundation Time due to Cumulative Restoration within the River')),
                             tag_levels = 'A',
...
ggsave("Figures/inun_ts.png", dpi=500, width = 16, height = 4, units="in")
```


#moving plot- river 1-7, Q0.5, 1st-80p
```{r}
...
profile1 <- RAS_1stO %>%
  group_by(STA)%>%
  filter(plan == "1st-80p" & storm == "Q0.5" & river == "1-7")%>% 
  mutate(
    ordinate = case_when(day == "01SEP2008" & time == "0000" ~0,
         TRUE~15))%>%
  mutate(ordinate = cumsum(ordinate))%>%
  mutate(
    ti_min = case_when(wettedFPw >= 15 ~ 1,
                       TRUE ~ 0))%>%
  filter(ordinate<=300)

profile1Plot <- profile1%>%
  ggplot(aes(STA, ti_min))+
  # geom_point(color = "blue")+
  geom_line(aes(group = ordinate), color = "blue")+
  scale_y_continuous(breaks=c(0,1))+
  scale_x_reverse()+ 
  theme(aspect.ratio=9/20)+
  transition_time(ordinate) +
  ggtitle("River 1-7, Q0.5, 1st-80p Floodplain inundation \n Minute: {round(frame_time)}")+
  labs(x = "Distance Upstream (ft)", y = "Water in Floodplain (1 = yes)")


animate(profile1Plot,
        nframes = 21,
        fps  = 1)
anim_save("moving_1_7Q0.5_1st80p.gif")

profile2 <- RAS_1stO %>%
  group_by(STA)%>%
  filter(plan == "1st-100p" & storm == "Q0.5" & river == "1-7")%>% 
  mutate(
    ordinate = case_when(day == "01SEP2008" & time == "0000" ~0,
         TRUE~15))%>%
  mutate(ordinate = cumsum(ordinate))%>%
...
profile2Plot <- profile2%>%
  ggplot(aes(STA, ti_min))+
  # geom_point(color = "blue")+
  geom_line(aes(group = ordinate), color = "green")+
  scale_y_continuous(breaks=c(0,1))+
  scale_x_reverse()+
  theme(aspect.ratio=9/20)+
  transition_time(ordinate) +
  ggtitle("River 1-7, Q0.5, 1st-100p Floodplain inundation  \n Minute: {round(frame_time)}")+
  labs(x = "Distance Upstream (ft)", y = "Water in Floodplain (1 = yes)")


animate(profile2Plot,
        nframes = 21,
        fps  = 1)
anim_save("moving_1_7Q0.5_1st100p.gif")


profile3 <- RAS_1stO %>%
  group_by(STA)%>%
  filter(plan == "1st-60p" & storm == "Q0.5" & river == "1-7")%>% 
  mutate(
    ordinate = case_when(day == "01SEP2008" & time == "0000" ~0,
         TRUE~15))%>%
  mutate(ordinate = cumsum(ordinate))%>%
...
profile3Plot <- profile3%>%
  ggplot(aes(STA, ti_min))+
  # geom_point(color = "blue")+
  geom_line(aes(group = ordinate), color = "orange")+
  scale_y_continuous(breaks=c(0,1))+
  scale_x_reverse()+
  theme(aspect.ratio=9/20)+
  transition_time(ordinate) +
  ggtitle("River 1-7, Q0.5, 1st-60p Floodplain inundation  \n Minute: {round(frame_time)}")+
  labs(x = "Distance Upstream (ft)", y = "Water in Floodplain (1 = yes)")


animate(profile3Plot,
        nframes = 21,
        fps  = 1)
anim_save("moving_1_7Q0.5_1st60p.gif")

profile4 <- RAS_1stO %>%
  group_by(STA)%>%
  filter(plan == "1st-100p" & storm == "Q1" & river == "1-7")%>% 
  mutate(
    ordinate = case_when(day == "01SEP2008" & time == "0000" ~0,
         TRUE~15))%>%
  mutate(ordinate = cumsum(ordinate))%>%
...
profile4Plot <- profile4%>%
  ggplot(aes(STA, ti_min))+
  # geom_point(color = "blue")+
  geom_line(aes(group = ordinate), color = "black")+
  scale_y_continuous(breaks=c(0,1))+
  scale_x_reverse()+
  theme(aspect.ratio=9/20)+
  transition_time(ordinate) +
  ggtitle("River 1-7, Q1, 1st-100p Floodplain inundation  \n Minute: {round(frame_time)}")+
  labs(x = "Distance Upstream (ft)", y = "Water in Floodplain (1 = yes)")


animate(profile4Plot,
        nframes = 21,
        fps  = 1)
anim_save("moving_1_7Q1_1st100p.gif")
```
...
inunVSrem$storm <- factor(inunVSrem$storm,                 # Re-level group factor
                         levels = c("Q2", "Q1", "Q0.5", "Qm"))
test$storm <- factor(test$storm,                 # Re-level group factor
                         levels = c("Q2", "Q1", "Q0.5", "Qm"))
# inunVSrem %>%
test%>%
  # filter(plan == "unr")%>%
  ...
  ggplot()+
  geom_point(aes(ti_hr, removal_scaled_kg, color = storm, shape = order))+
  # xlim(0,20)+
  # ylim(0,30)+
  xlab("Inundation time (hrs)")+
  # scale_x_log10()+
  scale_y_continuous(
    name = expression("Kilograms NO" [3]*" Removed (k" [m]*" rate)"),
    # limits = c(0, 30),
    sec.axis = sec_axis(trans=~.*21.31146981, name= expression("Kilograms NO" [3]*" Removed (k" [90]*" rate)")))+
  labs(color = "Storm", shape = "Inundation Location")+
  ggtitle(expression(atop("Inundation Time vs Nitrate Load Removed")))+
  plot_annotation(theme = theme(title = element_text(size = 12, face = 2))) &
  theme(text = element_text(size = 10))

ggsave("Figures/3141restoreOnly_inun_ts_vsremoval.png", dpi=500, width = 7, height = 4, units="in")
...
avg_inun_1st <- avg_inun%>%
  ...
  scale_x_continuous(breaks=seq(0,100,by=20))+
  xlab(expression(atop("Percent 1"^st*"-order streams restored")))+
  ylab(expression(atop("Average XS Inundation Time (hr)")))+
  ggtitle(expression("1"^st*"-order River"))
...
ggsave("Figures/avgInun_timesAllXS.png", dpi=500, height = 4, width = 16,  units="in")
```
...
```
#Calculating inundation time for full restoration the most recent way 09/28/23. Do this if have time or are asked.
```{r}
full_ti <- RAS_Full%>%
  group_by(time, river, storm, plan)%>% 
  mutate(
    inun_ct = case_when(
      wettedFPw >10 ~ 1, #giving a unit value to every timestep & XS 
      TRUE~0))%>%
...
full_ti%>%
ggplot()+
  geom_line(aes(PerShedRest, ti_hr, color = storm, group = storm))+
  geom_point(aes(PerShedRest, ti_hr, color = storm, shape = restLoc), size = 1)+
  # facet_wrap( ~restLoc, scales = "free", ncol = 4)+
