...
```{r plot-wider, fig.width=7, fig.height=5}
plot_Kmkm <- NO3_km %>%
  ...
  xlab(expression("Location of 1km restoration", "along the flowpath (km)"))+
  scale_y_continuous(
    name = expression("Percent 4"^th*"-order Watershed Load Removed (k"[m]*"rate)"),
    sec.axis = sec_axis(trans=~.*21.31146981, name= expression("Percent 4"^th*"-order Watershed Load Removed (k"[90]*"rate)")))+
  labs(color = "Storm", shape = "Areal Location")+
  ggtitle(expression(atop("Percent 4"^th*"-order Watershed NO"[3]*" Removed", "with 1 km Length Restoration Implemented")))+
...
ggsave("Figures/1km_2axis.png", width = 7, height = 5, dpi=500)
```
