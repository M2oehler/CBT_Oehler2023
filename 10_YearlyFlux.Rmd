...
yr_shedRem <- arrange(yr_shedRem, desc(sort_storm), sort_order, percentRest) %>%
  ...
  mutate(add_baseLoad_kg = 30865,
         ...
         restLoc = 
           case_when(startsWith(plan, "unr") ~ "1st-order Restoration",
                     startsWith(plan, "1st") ~ "1st-order Restoration",
                     startsWith(plan, "2nd") ~ "2nd-order Restoration",
                     startsWith(plan, "3rd") ~ "3rd-order Restoration",
                     startsWith(plan, "4th") ~ "4th-order Restoration"))
...
yr_ADDunr <- data.frame(plan = c("unr", "unr","unr"),
                       ...
                       restLoc = c("2nd-order Restoration", "3rd-order Restoration", "4th-order Restoration"))
