...
Ainun_1stO <- inun_1stO %>% #contains DF of XS and their associated inundation time over the simulation
  filter(ti_tot_min > 0)%>% #filter out XS that dont become inundated
  mutate( #Calculating area by first assigning widths do each station (differs by plan)
    FP_width =
           case_when(
  ...
  #                                     ~ (ti_tot_min/60)*(Xcs/2)*(FP_width*2)*k10/1000)) %>%
   mutate( #associated removal for the inundated XS
    km_removal_g =
      case_when(river == "1-7" & STA %in% c(165500,159500)
...
```{r}
#testing
testA <- bind_rows(Ainun_1stO, Ainun_2ndO, Ainun_3rdO, Ainun_4thO)%>%
  filter(scaleRemoval_km_kg>0)%>%
  mutate(
    restLoc = case_when(startsWith(plan, "1st")~"1-7",
                        startsWith(plan, "2nd")~"2-2",
                        startsWith(plan, "3rd")~"3-1",
                        startsWith(plan, "4th")~"4-1"))%>%
  filter(river == restLoc)


# bind_rows(Ainun_1stO, Ainun_2ndO, Ainun_3rdO, Ainun_4thO)%>%

#These include river areas only for scenarios where restoration occured in that river
testA%>%
  mutate(restLoc = 
           case_when(startsWith(plan, "unr") ~ "unr",
                     startsWith(plan, "1st") ~ "1st",
  ...
  ggtitle(expression(atop("Inundated Area vs Nitrate Load Removed")))+
  plot_annotation(theme = theme(title = element_text(size = 12, face = 2))) &
  theme(text = element_text(size = 10))

ggsave("Figures/restoreOnly_Avsremoval_km.png", dpi=500, width = 7, height = 4, units="in")
```


#Plot: total inundated FP area
```{r plot-wider, fig.width=13, fig.height=4}
Ainun_1st <- Ainun_1stO %>%
  filter(river =="1-7")%>%
  ggplot()+
  geom_point(aes(percentRest, scale_inun_A, color=storm))+
  geom_line(aes(percentRest, scale_inun_A, color=storm))+
  scale_x_continuous(breaks=seq(0,100,by=20))+
  ylim(0,13)+
