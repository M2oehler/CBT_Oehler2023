...
```{r}
#Make vector of file names, then convert to DF and include the location to use later to read in the files.I just changes the name of the dataframe and folders to load the other order restoration files.
raw.files <- tibble(filename = list.files("TextFiles2ndO/"))
raw.file.paths <- raw.files  %>%
  mutate(filepath = paste0("TextFiles2ndO/", filename))
...
for (f in raw.file.paths$filepath) {
  dat <- read_fwf(f, 
         ...
         mutate(plan = f)
   RAS_2ndO <- bind_rows(RAS_2ndO, dat)
}
...
RAS_2ndO$STA <- as.numeric(RAS_2ndO$STA)
rm(dat, raw.file.paths, raw.files)
```


#Save data frames to later be called back into the environment
```{r}
# save(RAS_1stO, RAS_2ndO, RAS_3rdO, RAS_4thO,
#     file = "C:/Users/morganao/Documents/CBT_Oehler2023/9_Saved_Dataframes/SO_RAS_rawData.RData")

#Contains data for load removal (g), watershed removal(%), and local removal (%) for 3 removal rates and 3 source concentrations.
# save(NO3_1stO, NO3_2ndO, NO3_3rdO, NO3_4thO,
#     file = "C:/Users/morganao/Documents/CBT_Oehler2023/9_Saved_Dataframes/SO_NO3_removals.RData")

#Ran the nitrate code partially to save dataframes of just the inundation times
# save(inun_1stO, inun_2ndO, inun_3rdO, inun_4thO,
#     file = "C:/Users/morganao/Documents/CBT_Oehler2023/9_Saved_Dataframes/SO_InunTimes.RData")
```



#%NO3 Removal by 1st Order River Restoration. Produces load removals in the entire order (runtime: ~3min if have raw data loaded)
```{r}
...
NO3_1stO <- RAS_1stO %>%
  ...
  mutate(
    FP_width = 
           case_when(
             ...
             river == "3-1" ~ 67.5,
             river == "4-1" ~ 126)) %>% 
  mutate(#calculates removal at each XS along the flow path
    k10_removal_g = 
  ...
  summarise(removal_k10_g = sum(k10_removal_g), #sums removal at each XS in a river
            removal_km_g = sum(km_removal_g),
            removal_k90_g = sum(k90_removal_g)) %>%
  mutate(removal_k10_g = case_when(river == "1-7" ~ removal_k10_g*24, #scales removal to the number of rivers in the order
...
#%NO3 Removal by 2nd Order River Restoration. Produces load removals in the entire order.
```{r}
#Realised the single order 2nd-40p was also unstable so removing it from the dataframe
# NO3_2ndO <- RAS_2ndO[!(RAS_2ndO$plan=="2nd-40p"),]
...
#%NO3 Removal by 3rd Order River Restoration. Produces load removals in the entire order.
```{r}
#Nitrate loads for 1mg/L source concentration(g)
Q2_watershed_load <- 4563108 #total NO3 load to the watershed (grams)
Q2_load1 <- 141902 #input load to a all first order streams (grams)
...
#%NO3 Removal by 4th Order River Restoration. Produces load removals in the entire order.
```{r}
#Nitrate loads for 1mg/L source concentration(g)
Q2_watershed_load <- 4563108 #total NO3 load to the watershed (grams)
Q2_load1 <- 141902 #input load to a all first order streams (grams)
