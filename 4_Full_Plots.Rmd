...
NO3_Full %>%
  ...
  labs(x = expression("Percent of 4"^th*"-order Watershed Restored"),
       color = "Storm",
       shape = expression(atop("Location of","Restoraion")))+
  scale_y_continuous(
    name = expression("Percent NO"[3]*" Removed from the Watershed (k"[m]*")"),
    limits = c(0, 1.25),
    sec.axis = sec_axis(trans=~.*21.31146981, name= expression("Percent NO"[3]*" Removed from the Watershed (k"[90]*")"), breaks = seq(0, 25, 5)))+
  scale_x_continuous(breaks=seq(0,100,by=20),  sec.axis = sec_axis(~.*130.454/100, name = "Total Watershed Length Restored [km]", breaks = seq(0,135,10)))+
  ggtitle(expression("Cumulative Restoration of the Full Watershed: %NO"[3]*" Removed at the 4"^th*"-order Outlet"))
ggsave("Figures/Full_2axis.png", dpi=500, height = 5, width = 8, units="in")
